...
TechWordLearn 会把你在日常阅读里反复点读的生词排到前面。

> **记住一个，下一批自然顶上来。**

不用先背一堆“可能有用”的词。出现得多，说明实用；点读得多，说明你还不会。

![点击 Chrome 右上角的 TechWordLearn 图标查看点读次数](docs/showcase/techwordlearn-cover.png)

### 只需要记住 4 件事

1. **先看最值得记的词**：每次点击一个高亮单词，点读次数都会自动加一。点击 Chrome 右上角的 TechWordLearn 图标，排在前面的就是当前最值得记的词。
2. **加新词**：选中网页里的英文单词，点右键，再点击“把这个词加入我的词库”。
3. **学会了**：Mac 按住 `Option` 点击高亮词，Windows 按住 `Alt` 点击高亮词，确认后取消高亮。
4. **查单词**：鼠标放在高亮词上看中文，点击单词听发音。
...
- 选择“导出备份”，保存自己添加或修改过的词以及已隐藏的词。
- 选择“导入备份”，将备份内容合并到当前词库；同名单词会被导入内容覆盖。
