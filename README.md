...
1. **先看最常查的词**：每次点击一个高亮单词，查询次数都会自动加一。点击 Chrome 右上角的 TechWordLearn 图标，就能看到每个词查了多少次。
2. **加新词**：网页里遇到想学的新单词，选中它，点右键就能加入词库。
3. **学会了**：Mac 按住 `Option` 点击单词，Windows 按住 `Alt` 点击单词，确认后取消高亮。
4. **随手查单词**：常见技术词会自动高亮；鼠标放上去看中文，点击听发音。

最简单的学习方法只有一句话：

> 看到不熟的高亮单词 → 点击查询 → 打开右上角插件 → 看哪些词查得最多

**次数越高，说明你越常遇到或越容易忘，越值得优先学习。**

遇到词库里没有的新单词：

> 选中英文单词 → 点右键 → 点击 `Add "..." to Tech Vocabulary`（加入词库）→ 填写中文意思

已经学会一个词：

> Mac：按住 `Option` 点击高亮单词；Windows：按住 `Alt` 点击高亮单词 → 确认“已掌握” → 取消高亮

## 第一次使用：5 步就够

1. [先按这里的步骤安装扩展](#本地安装)。
2. 打开一个英文技术网页，例如英文帮助文档或技术文章。
3. 刷新网页，找到被高亮的单词。
4. 鼠标放上去看中文，点击单词听发音；查询次数会自动加一。
5. 点击 Chrome 右上角的 TechWordLearn 图标，查看刚才的查询次数。

想查看学过的词，点击 Chrome 右上角的 TechWordLearn 图标，再点击 `Open Vocabulary Manager`（打开词库管理）。

如果网页里没有出现高亮，先点击扩展图标，确认上方显示 `TechWordLearn enabled`（已启用），然后刷新网页。
...
TechWordLearn 是一款面向中文用户的开源 Chrome 扩展。每次点击高亮词时，它都会记录查询次数；点击 Chrome 右上角的插件图标，就能看到自己最常查、最值得优先学习的技术词汇。同时，它也提供网页高亮、中文释义和发音。
...
- 点击 Chrome 右上角的插件图标，立即查看按频率排列的查询次数
- 每次点击高亮词自动累计查询次数，帮助找出反复卡住的词
- 在词库中默认把查询次数最多的词排在前面
- 在网页正文中高亮技术英语词汇
...
- 查看累计查询和自然周查询统计
- 选中网页里的英文单词，通过右键菜单直接加入词库
- 管理自定义词库：新增、修改、删除、隐藏和恢复词条
- 将已掌握词汇取消高亮：Mac 使用 `Option + 点击`，Windows 使用 `Alt + 点击`
- 自动保留词库备份和版本快照
- 导入或导出备份；文件格式为 JSON
...
3. 将鼠标移到高亮词汇上查看中文释义。
4. 点击词汇播放发音。
5. 点击 Chrome 右上角的插件图标，查看按查询次数排列的高频词。
6. 打开“词库管理”，新增一个自定义词。
7. 刷新网页，确认新词已进入高亮范围。
...
1. 用鼠标选中网页里的英文单词。
2. 点击鼠标右键。
3. 点击 `Add "..." to Tech Vocabulary`（加入词库）。
4. 填写中文意思并确认。

新词保存后会进入自定义词库，并在支持的网页中参与高亮。
...
在扩展弹窗中点击 `Open Vocabulary Manager`（打开词库管理）。主页面用于快速搜索和浏览当前词库：
...
点击 Chrome 右上角的 TechWordLearn 图标，可以立即看到按查询次数排列的高频词。弹窗优先显示当前自然周；如果本周还没有查询记录，则显示累计记录。

在词库管理页点击“查看统计 ›”，可以进一步查看当前自然周内的高频查询词。
