...
test("active product surfaces use the reading-driven dynamic vocabulary definition", () => {
  ...
  const humanSlogan = "## 越常用 / 越没记住 → 越在你眼前 → 直到记住";
  assert.match(firstScreen, /^[ \t]*TechWordLearn$/m);
  assert.doesNotMatch(firstScreen, /^# TechWordLearn$/m);
  assert.equal(firstScreen.split("\n").filter((line) => line === humanSlogan).length, 3);
  assert.match(firstScreen, /操，这个词我明明见过好多次，怎么又不认识。/);
  assert.match(firstScreen, /TechWordLearn 专门盯住这种词。/);
  assert.match(firstScreen, /## 看一遍就会：3 步/);
  assert.match(firstScreen, /## 1\. 遇到生词，加入/);
  assert.match(firstScreen, /## 2\. 越点，越靠前/);
  assert.match(firstScreen, /## 3\. 记住了，让它消失/);
  assert.doesNotMatch(firstScreen, /^### [123]\./m);
